# Stack Decisions — nextjs-container
...
> **This is the `nextjs-container` preset** — the documented warm-Node deviation of `nextjs-default`, instantiating two of pinned-decisions' deviation rules: **ORM → Prisma** (warm server valuing the most mature migration tooling) and **Hosting → container PaaS** (a long-running Node server in a container — Railway/Fly/Render — box 2 of the three-box model). Everything else (TypeScript-strict, Biome, Vitest/Testing-Library/MSW, Playwright, Zod, Better Auth, pnpm, hooks, CI shape) is identical to `nextjs-default`. Only the ORM (S10), the framework deployment note (S17), and hosting (S18) differ from that preset, plus the container build-provenance ADR (S19) below.
...
## ADR-S10: Prisma as ORM (the warm-Node deviation)

- **Decision:** Prisma (`@prisma/client` + the `prisma` CLI) for schema, queries, and migrations; schema in `prisma/schema.prisma`, migrations in `prisma/migrations/**`.
- **Rationale:** This preset runs a **warm, long-running Node server** in a container, so the cold-start advantage that makes Drizzle the `nextjs-default` pick does not apply — and the ORM decision rule (`00-governance/pinned-decisions.md`) routes the warm-server case to Prisma for its **most mature migration tooling** (declarative migrations with drift detection, `migrate diff`, shadow-DB verification). Generated client + typed queries; `prisma migrate deploy` is forward-only and idempotent, run on container start.
- **Accepted tradeoff:** A code-generation step (`prisma generate`, wired into the Docker build) and a DSL schema file rather than plain-TypeScript — less directly AI-editable than Drizzle, but the maturity of the migration engine is the point on a warm server. **NEVER `prisma db push` in production** (prototyping only) — same discipline as `06-delivery/migration-discipline.md`.
...
## ADR-S18: Container hosting for a warm Node server (the hosting deviation)

- **Decision:** Build a container image (multi-stage `Dockerfile`, Next.js `output: 'standalone'`) and deploy it to a container PaaS — Railway / Fly.io / Render. Migrations (`prisma migrate deploy`) run on container start; production from `main`, preview env per PR where the PaaS supports it.
- **Rationale:** This is box 2 of the three-box model (`00-governance/pinned-decisions.md`) — chosen when the app is a **warm, long-running server** (WebSockets, cron, background jobs, or simply preferring a persistent process over serverless cold starts). A container is the portable, host-agnostic unit; `standalone` output keeps the image minimal.
- **Accepted tradeoff:** You own the Dockerfile, base-image patching, and (unlike Vercel's zero-config) the deploy wiring; in exchange you get no serverless limits and host portability. Pin the base image (and ideally by digest in production) per supply-chain verify-on-consume.

## ADR-S19: Build-provenance attestation on the container image

- **Decision:** The release pipeline builds + pushes the image and records a **SLSA build-provenance attestation** (`actions/attest-build-provenance`) bound to the image digest, pushed to the registry.
- **Rationale:** A container image is a published artifact, so — unlike the serverless `nextjs-default` preset — there IS something to attest. Provenance is the unforgeable link from image digest back to the workflow that built it (`04-build/supply-chain.md`); consumers/deployers can verify it before running. Completes the supply-chain posture the global doc prescribes.
- **Accepted tradeoff:** Requires registry write + keyless-signing permissions in the release workflow; the attestation only has value if the deploy step actually verifies it (a host-side step the project wires).
