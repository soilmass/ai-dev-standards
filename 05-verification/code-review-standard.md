...
- [ ] Confirm every released artifact carries signed SLSA-provenance + an SBOM generated in CI, that external build inputs (base images, actions, binaries) are pinned by digest and verified before use, and that verification failure blocks the pipeline. (`04-build/supply-chain.md`)
