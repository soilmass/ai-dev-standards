...
6. **Auth schema:** Better Auth needs four models (`User`/`Session`/`Account`/`Verification`). Generate them with `pnpm dlx @better-auth/cli generate` (Prisma output), merge into `prisma/schema.prisma`, and **reconcile column names to snake_case** via `@map`/`@@map` (data-modeling rule 1). Make your domain models' `userId` a real relation to `User`.
7. **Database:** `prisma/schema.prisma` (starter: snake_case via `@map`, UUIDv7 PK via `@default(uuid(7))`, timestamptz, soft-delete as a per-model decision) and `lib/db.ts` (the server-only Prisma singleton) are injected. Adapt the schema, set `DATABASE_URL` in `.env.local`, then `pnpm exec prisma migrate dev` for the first migration. Migrations land in `prisma/migrations/**` — the CI guard watches that path (forward-only; **never `prisma db push` in production**). Use queries via `getDb()`.
...
10. **After first push:** set branch protection per `05-verification/ci-pipeline.md` (Bootstrap section) — all PR jobs as required checks.
