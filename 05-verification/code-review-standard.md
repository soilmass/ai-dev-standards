...
- [ ] Tokens cover the DTCG groups (color roles, spacing, sizing/radii, typography, motion, elevation/border) and are defined once and aliased — no bare hex or magic numbers in components; and every non-trivial visual/interaction decision cites the specific HIG / Material 3 / WAI-ARIA APG section it follows, recording any platform convention borrowed or rejected (web a11y norms override the platform on conflict). (`03-design/design-tokens-and-hig.md`)
- [ ] Every page's OG/social-card and the site's icons and illustrations form one coherent visual language — OG cards share a template, dimensions, and palette across pages (no stretched-logo or framework-default fallback), and icons/illustrations come from one set/style rather than a mismatched grab-bag. (`03-design/graphics-asset-cohesion.md`)
- [ ] Every content image's alt text conveys the image's purpose in context (the chart's takeaway, the logo's company name) rather than a filename, "image", or keyword stuffing, and every decorative image is correctly empty-alt — presence of the attribute is lint-checked; its meaning is verified by reading it against the surrounding content. (`03-design/graphics-asset-cohesion.md`)
- [ ] Confirm the layout is authored mobile-first (base styles target the smallest viewport; breakpoints are ascending min-width additions placed at content-reflow points), all spacing/gaps come from the documented scale (no arbitrary pixel values, structure via grid/flex not magic offsets), reading columns are capped to a ~45–75-character measure, line-height/letter-spacing are set per type role, and the surface has been verified — by visual baseline or by hand — at a small-phone, a tablet, and a desktop width, not just one. (`03-design/responsive-layout-discipline.md`)
