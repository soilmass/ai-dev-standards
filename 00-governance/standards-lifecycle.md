...
| State | Meaning |
|---|---|
| **Active** | In force. Linked from `00-governance/completeness-matrix.md`. |
...
1. Confirm the gap is real: check `00-governance/completeness-matrix.md` — is the cell empty or marked as a gap?
...
4. Update `00-governance/completeness-matrix.md` and run `scripts/audit-completeness.sh`.
...
- How this repo's own tagging/signing/release practice relates to the git/GitHub standard it ships is recorded rule-by-rule in `00-governance/repo-operating-model.md`.

## 5. Deprecating a standard

1. Mark the doc `DEPRECATED` with a pointer to its replacement (or to the rationale if there is none).
2. Update `00-governance/completeness-matrix.md` so the cell points at the replacement.
...
1. **During a project**, friction with a standard is logged in the project's debt-log "Library flow-back" section (the template ships one) — not fixed by editing the library mid-task (agents: see the never-touch list in `00-governance/agent-operating-rules.md`).
2. **After the work ships** (or at a currency pass), each flow-back finding gets a row in `00-governance/flow-back-log.md` with a **disposition**: `patched` (a doc/preset/ADR change shipped under a named tag), `deferred` (with a revisit trigger), or `no-change` (with a one-line reason). No finding is dropped silently; `scripts/check-flowback.sh` keeps the ledger honest (patched rows must name a real tag). A finding that moves a calibration knob also gets a row in `00-governance/calibration.md`'s Observations section.
3. **Currency pass**: at least twice a year, (a) re-verify the time-sensitive claims in `00-governance/pinned-decisions.md` and the dated guardrails in `00-governance/agent-operating-rules.md`; (b) for each known consuming project, diff its debt-log "Library flow-back" section against `00-governance/flow-back-log.md` and add any unprocessed finding (the library can't enumerate project locations, so this harvest is manual); (c) walk the calibration register — for any knob whose data a project actually touched this period, check its recalibration trigger against that data and record an observation; run `scripts/check-calibration.sh`. (Walk only the data-touched knobs, not all of them against absent data — an exhaustive walk with no signal is theater that gets skipped.) Update the snapshot date when done.
