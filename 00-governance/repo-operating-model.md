...
|---|---|---|
| Conventional Commits — `<type>(<scope>): <subject>`, ≤72 (`04-build/git-standards.md` rule 5) | **Follows** | History is already Conventional Commits; it keeps `git log` scannable and lets the tag/changelog discipline work. |
| One logical change per commit (`04-build/git-standards.md` rule 7) | **Follows** (as "one enforcement-altering batch per commit/tag") | Matches the phased-build expectation in `CLAUDE.md` §4 and the versioning rule below. |
| Trunk-based, short-lived branches + PRs; no direct commits to `main` (`04-build/git-standards.md` rules 1–4, 8–11) | **Waives** | Solo repo. A branch + PR for a one-person library with a single CI gate adds ceremony without a second reviewer. Proposals live as working-tree drafts the human reviews, then commits direct to `main` (`00-governance/standards-lifecycle.md` §2, §7). The `/batch`-style PR fan-out does not apply here. |
| Required PR checks + definition-of-done gate via branch protection (`05-verification/ci-pipeline.md`, `05-verification/code-review-standard.md`) | **Adapts** | No PRs, so the DoD/self-review discipline is applied by the human at commit time. `suite-ci.yml` runs `scripts/suite-ci.sh` on **push and PR** as the one authoritative gate (`CLAUDE.md`). |
...
| Tags `vYYYY.MM[.N]`, human-tagged, versioned-as-a-whole (`00-governance/standards-lifecycle.md` §4) | **Follows** | The repo is versioned as a whole by git history; tags mark enforcement-altering batches. `check-flowback.sh` already depends on `patched` rows naming real tags. |
...
| Generated changelog + GitHub Releases on tag (`06-delivery/release-process.md` rule 4) | **Adapts / N/A** | This repo ships no `release.yml` for itself: tags are the release markers, and the commit history *is* the changelog for a docs library. The release tooling is a consuming-project concern. |
...
| Dependency updates / Dependabot (`08-maintenance/dependency-updates.md`) | **N/A** | The repo has no package dependencies — only markdown, bash, and `.example` configs. Its CI uses a small set of pinned GitHub Actions, reviewed by hand. |
