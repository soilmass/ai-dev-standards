...
3. **Dev deps:** `pnpm add -D @biomejs/biome typescript tsx vitest @vitejs/plugin-react vite-tsconfig-paths @vitest/coverage-v8 jsdom @testing-library/react @testing-library/jest-dom @testing-library/user-event msw @playwright/test drizzle-kit husky lint-staged @commitlint/cli @commitlint/config-conventional`
...
6. **Test scaffolding:** create `tests/setup.ts` (jest-dom matchers + MSW server lifecycle), `tests/e2e/`, `tests/visual/`; generate visual baselines locally (`pnpm exec playwright test --project=visual --update-snapshots`) and commit them.
7. **Database:** create `db/schema.ts`, set `DATABASE_URL` in `.env.local`, then `pnpm exec drizzle-kit generate` for the first migration. `drizzle.config.ts` (already injected) pins migrations to `./drizzle` — the CI guard watches that path.
8. **Env validation at boot:** call `serverEnv()` from `instrumentation.ts` (or import it in `next.config`) so startup fails on bad config; client code imports `clientEnv` only.
