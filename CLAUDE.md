...
- **Every tunable knob lives in the calibration register.** Changing any threshold, budget, cadence, score floor, or version pin means updating its row (value + rationale + recalibration trigger) in `00-governance/calibration.md` — and its manifest entry — in the same commit. `scripts/check-calibration.sh` (run by suite CI) fails on any disagreement between the register and the tree, and on any manifest id that doesn't map to its table row. Adding a new knob means adding its row + manifest entry; a recalibration driven by real data also gets an Observations row.
- **Lessons from consuming projects flow back through the ledger.** A finding a project records in its debt-log "Library flow-back" section lands in `00-governance/flow-back-log.md` with a disposition (`patched`/`deferred`/`no-change`) — no silent drops. `scripts/check-flowback.sh` (suite CI) keeps it honest: `patched` rows must name a real tag. Process: `00-governance/standards-lifecycle.md` §6.
